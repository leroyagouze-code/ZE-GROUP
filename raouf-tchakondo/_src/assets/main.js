...
(function () {
  ...
  // ---------- carrousel 3D ----------
  // Rotation automatique lente, glisser (avec inertie), flèches, clavier, clic sur une carte.
  // Animations réduites / connexion lente : pas de rotation automatique ni d'inertie.
  $$('[data-g3d]').forEach(function (root) {
    var stage = $('.g3d-stage', root);
    var ring = $('.g3d-ring', root);
    var cards = $$('.g3d-card', root);
    var caption = $('.g3d-caption', root);
    var toggle = $('[data-g3d-toggle]', root);
    var n = cards.length;
    var step = 360 / n;
    var angle = 0, vel = 0, target = null;
    var dragging = false, moved = 0, lastX = 0;
    var paused = calm, hover = false, visible = false, raf = 0, shown = -1;

    var snap = function (a) { return Math.round(a / step) * step; };
    var front = function () { return ((Math.round(-angle / step) % n) + n) % n; };

    var render = function () {
      ring.style.setProperty('--a', angle.toFixed(2) + 'deg');
      cards.forEach(function (c, i) {
        var rel = ((((i * step + angle) % 360) + 540) % 360) - 180;
        c.style.setProperty('--f', (Math.abs(rel) / 180).toFixed(3));
      });
      var f = front();
      if (f !== shown) {
        shown = f;
        caption.textContent = '0' + (f + 1) + ' · ' + cards[f].getAttribute('data-title');
      }
    };
    ...
    var tick = function () {
      ...
      if (!dragging) {
        if (target !== null) {
          angle += (target - angle) * (calm ? 1 : 0.1);
          if (Math.abs(target - angle) < 0.05) { angle = target; target = null; }
        } else if (Math.abs(vel) > 0.02) {
          angle += vel;
          vel *= 0.94;
          if (Math.abs(vel) <= 0.02) { vel = 0; target = snap(angle); }
        } else if (!paused && !hover) {
          angle -= 0.09;
        }
      }
      render();
      if (visible && (!paused || target !== null || vel || dragging)) raf = requestAnimationFrame(tick);
    };
    var wake = function () { if (!raf && visible) raf = requestAnimationFrame(tick); };

    var go = function (dir) { vel = 0; target = snap(target !== null ? target : angle) - dir * step; wake(); };
    $('[data-g3d-next]', root).addEventListener('click', function () { go(1); });
    $('[data-g3d-prev]', root).addEventListener('click', function () { go(-1); });
    ...
    var release = function (e) {
      ...
      dragging = false;
      if (moved < 6) {
        // Clic : amène la carte touchée au premier plan
        var el = document.elementFromPoint(e.clientX, e.clientY);
        var card = el && el.closest('.g3d-card');
        if (card) {
          var i = cards.indexOf(card);
          var goal = -i * step;
          goal += Math.round((angle - goal) / 360) * 360;
          target = goal;
        }
        vel = 0;
      } else if (calm) {
        vel = 0; target = snap(angle);
      }
      wake();
    };
  ...
  // ---------- accueil : parallaxe du visuel ----------
  var px = $('[data-parallax]');
  if (px && !calm && window.matchMedia('(hover: hover)').matches) {
    var hero = px.closest('.hero') || px;
    hero.addEventListener('pointermove', function (e) {
      var r = hero.getBoundingClientRect();
      px.style.setProperty('--mx', (((e.clientX - r.left) / r.width) * 2 - 1).toFixed(3));
      px.style.setProperty('--my', (((e.clientY - r.top) / r.height) * 2 - 1).toFixed(3));
    });
    hero.addEventListener('pointerleave', function () {
      px.style.setProperty('--mx', 0);
      px.style.setProperty('--my', 0);
    });
  }
